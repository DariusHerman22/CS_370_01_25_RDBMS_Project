<html>
<head>
    <title>EpicAwesomeStore</title>
    <link href="css/bootstrap.css" rel="stylesheet" />
    <script src="js/bootstrap.bundle.js"></script>
</head>
<body>

<ul class="nav">
    <li class="nav-item">
        <a class="nav-link" href="index.php">EpicAwesomeStore Home</a>
    </li>
    <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">Data Import</a>
        <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="import_pizza_data.php">Import Customer Data</a></li>
            <li><a class="dropdown-item" href="#">Import Vendor Data</a></li>
            <li><a class="dropdown-item" href="#">Import Product & Shopping Cart Data</a></li>
        </ul>
    </li>
    <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">Reports</a>
        <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="pizza_data_report.php">Customer Report</a></li>
            <li><a class="dropdown-item" href="#">Vendor Report</a></li>
            <li><a class="dropdown-item" href="#">Product & Shopping Cart Report</a></li>
        </ul>
    </li>
</ul>

<div class="container">